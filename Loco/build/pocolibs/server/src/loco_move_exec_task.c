...
/* maximum move mbox size */
static __inline__ size_t
loco_move_mbox_size(void)
{
  return 0;
}


/* --- genom_loco_move_exec_task ---------------------------------------- */

void *
genom_loco_move_exec_task(void *data)
{
  enum {
    GENOM_SCHED_DONE = 1,
    GENOM_SCHED_ONE = 2,
  ...
  genom_state_rusage *usage = &self->tasks.move.rusage;
  ...
  timer = h2timerAlloc();
  if (timer == NULL) {
    genom_log_warn(1, "cannot create timer for %s exec task", "move");
    goto signal_parent;
  }

  if (h2timerStart(timer, 1, 0) != OK) {
    genom_log_warn(1, "cannot start timer for %s exec task", "move");
    goto signal_parent;
  }

  /* initialize csLib for remotes */
  if (loco_move_mbox_size()) {
    char mbox[64];
    snprintf(mbox, sizeof(mbox), "%s/%s", genom_instance, "move");

    if (csMboxInit(mbox, 0, loco_move_mbox_size()) == ERROR) {
  ...
  /* run */
  genom_log_info("spawned task move");
  ...
  do {
    ...
    if (!(sched & GENOM_SCHED_CONT)) {
      ...
      gettimeofday(&tvs, NULL);
      genom_log_debug("move tick");
    }
    pthread_mutex_lock(&self->tasks.move.lock);
    /* paused activities are only awaken on a period tick */
    self->tasks.move.runnable = !(sched & GENOM_SCHED_CONT);
    sched = 0;

    if (self->tasks.move.shutdown) {
    ...
    for(id = 0; id < genom_max_activities(); id++) {
      ...
      switch(a->status) {
        ...
        case ACT_INIT:
          if (a->start && self->tasks.move.runnable) {
          ...
          if (a->stop) {
            ...
            if (a == self->tasks.move.permanent) {
              ...
              d.by[sizeof(d.by)-1] = *"";
              genom_interrupted(&d, &self->tasks.move.context);

              a->status = ACT_ETHER;
              a->stop = 0;
              a->exdetail = (void *)self->tasks.move.context.raised(
                &a->state, &self->tasks.move.context);
        ...
        case ACT_RUN:
          ...
          /*FALLTHROUGH*/
        case ACT_STOP:
          if (a->pause && self->tasks.move.runnable) {
            a->pause = 0;
          }
          break;
      }
    }
    self->tasks.move.runnable = 0;
    ...
    for(id = 0; id < genom_max_activities(); id++) {
      ...
      pthread_mutex_unlock(&self->tasks.move.lock);
      e = genom_loco_move_invoke(self, a);
      pthread_mutex_lock(&self->tasks.move.lock);
      switch(e) {
        case ACT_RUN:
          if (!a->pause)
  ...
  genom_log_info("shutting down %s exec task", "move");
