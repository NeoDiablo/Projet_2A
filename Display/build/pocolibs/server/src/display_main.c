...
const char *genom_instance = "display";
...
static const char usage_string[] = "\
  ...
  -i,--name=string  set component instance name\n\
     ...
     --version      display display version number\n\
...
static void	genom_display_run(void);
static void	genom_display_vacuum(const char *pidfile);
...
genom_display_run(void)
{
  ...
  if (pid_file_fd < 0) {
    if (force && errno == EEXIST) {
      ...
      genom_log_info("forcibly disabling the protection against duplicate"
                     " instances");
      genom_display_vacuum(pid_file_path);
      pid_file_fd = open(pid_file_path, O_CREAT|O_EXCL|O_WRONLY, 0644);
    }
    if (pid_file_fd < 0) {
      genom_log_warn(-1, "error creating %s", pid_file_path);
  ...
  display_self = genom_display_init();
  if (!display_self) {
  ...
  genom_display_fini(display_self);
  unlink(pid_file_path);
}


/* --- genom_display_vacuum --------------------------------------------- */

static void
genom_display_vacuum(const char *pidfile)
{
