...
void threadVideo(void *arg)
{
	...
	if(rt_task_set_periodic(NULL, TM_NOW, rt_timer_ns2ticks(200000000)) != STATUS_OK)
	...
	else
	{
		...
		do
		{
			...
			//envoie position dans msgGui
			{
				...
				rt_queue_send(&queueMsgGUI, msg_serv, MSG_QUEUE_SIZE, Q_NORMAL); // a v�rifier
				...
				rt_queue_send(&queueMsgGUI, msg_pos, MSG_QUEUE_SIZE, Q_NORMAL); // a v�rifier
			}
			...
			//envoie de l'image dans msgGUI
			{
				...
				rt_queue_send(&queueMsgGUI, msg_img, MSG_QUEUE_SIZE, Q_NORMAL); // a v�rifier
			}
