...
/* maximum film mbox size */
static __inline__ size_t
camera_film_mbox_size(void)
{
  return 0;
}


/* --- genom_camera_film_exec_task -------------------------------------- */

void *
genom_camera_film_exec_task(void *data)
{
  enum {
    GENOM_SCHED_DONE = 1,
    GENOM_SCHED_ONE = 2,
  ...
  genom_state_rusage *usage = &self->tasks.film.rusage;
  ...
  timer = h2timerAlloc();
  if (timer == NULL) {
    genom_log_warn(1, "cannot create timer for %s exec task", "film");
    goto signal_parent;
  }

  if (h2timerStart(timer, 1, 0) != OK) {
    genom_log_warn(1, "cannot start timer for %s exec task", "film");
    goto signal_parent;
  }

  /* initialize csLib for remotes */
  if (camera_film_mbox_size()) {
    char mbox[64];
    snprintf(mbox, sizeof(mbox), "%s/%s", genom_instance, "film");

    if (csMboxInit(mbox, 0, camera_film_mbox_size()) == ERROR) {
  ...
  /* run */
  genom_log_info("spawned task film");
  ...
  do {
    ...
    if (!(sched & GENOM_SCHED_CONT)) {
      ...
      gettimeofday(&tvs, NULL);
      genom_log_debug("film tick");
    }
    pthread_mutex_lock(&self->tasks.film.lock);
    /* paused activities are only awaken on a period tick */
    self->tasks.film.runnable = !(sched & GENOM_SCHED_CONT);
    sched = 0;

    if (self->tasks.film.shutdown) {
    ...
    for(id = 0; id < genom_max_activities(); id++) {
      ...
      switch(a->status) {
        ...
        case ACT_INIT:
          if (a->start && self->tasks.film.runnable) {
            /* start request, only on a tick in periodic tasks */
            a->status = ACT_RUN;
            a->state = camera_start;
            a->start = 0;
            a->pause = 0;
          }
          if (a->stop) {
            ...
            if (a == self->tasks.film.permanent) {
              ...
              d.by[sizeof(d.by)-1] = *"";
              genom_interrupted(&d, &self->tasks.film.context);

              a->status = ACT_ETHER;
              a->stop = 0;
              a->exdetail = (void *)self->tasks.film.context.raised(
                &a->state, &self->tasks.film.context);
        ...
        case ACT_RUN:
          ...
          /*FALLTHROUGH*/
        case ACT_STOP:
          if (a->pause && self->tasks.film.runnable) {
            a->pause = 0;
          }
          break;
      }
    }
    self->tasks.film.runnable = 0;
    ...
    for(id = 0; id < genom_max_activities(); id++) {
      ...
      pthread_mutex_unlock(&self->tasks.film.lock);
      e = genom_camera_film_invoke(self, a);
      pthread_mutex_lock(&self->tasks.film.lock);
      switch(e) {
        case ACT_RUN:
          if (!a->pause)
  ...
  genom_log_info("shutting down %s exec task", "film");
